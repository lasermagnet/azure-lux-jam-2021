...
export let gPFConst = {
	...
	kUpdateUserTitleDisplayName: "UpdateUserTitleDisplayName"
};
...
export async function PlayFab_UpdateDisplayName(displayName)
{
	let pfRequest = {
		DisplayName: displayName
		};
	...
	const playfabPromise = (pfRequest) => 
	{
  		return new Promise((resolve, reject) => 
		{
  			PlayFabClientSDK.UpdateUserTitleDisplayName(pfRequest, (result, error) => 
			{
				if(error) return reject(error);
				resolve(result);
			});
	...
	gPlayFabCachedData.Results[gPFConst.kUpdateUserTitleDisplayName] = { Result: null, Error: null};
	
	return playfabPromise(pfRequest).then(
		result => {
			gPlayFabCachedData.Results[gPFConst.kUpdateUserTitleDisplayName].Result = result;
		}
	).catch(
		error => {
		gPlayFabCachedData.Results[gPFConst.kUpdateUserTitleDisplayName].Error = error;
		console.log("Error:"+CompileErrorReport(error));
		}
	);
}
